...
gg=find(abs(H)<30&abs(Ta)<60&abs(Ts)<60&hr==(j-1)&isDS==1&abs(Ts)>2&ustar>0); %find dormant season data collected when H is small, Ta and Ts make sense, 
...
%suitable data
XTDS(j)=nanmean(Ta(gg))./nanmean(Ts(gg)); %save the ratio X_T for each hour of the day, during the growing season
sXTDS(j)=nanmean(Ta(gg))./nanmean(Ts(gg))./sqrt(length(gg)); %find the standard error of the mean for X_T

hh=find(hr==(j-1)&isDS==1); %find all dormant season data for the jth hour
Taero(hh)=XTDS(j).*Ts(hh); %find the Taero for all time periods as the product of X_T and Tsurf

%now, repeat for the growing season
gg=find(abs(H)<30&abs(Ta)<60&abs(Ts)<60&hr==(j-1)&isGS==1&abs(Ts)>2&ustar>0); 
XTGS(j)=nanmean(Ta(gg))./nanmean(Ts(gg));
sXTGS(j)=nanmean(Ta(gg))./nanmean(Ts(gg))./sqrt(length(gg)); 
hh=find(hr==(j-1)&isGS==1); 
Taero(hh)=XTGS(j).*Ts(hh); 
